...
CrossTab ,Rpt = RF(df,'Species',500) 
print(Rpt)
print(CrossTab)


CrossTab ,Rpt = RF(df,'Species',estimators= 999,
                    min_samples = 5,
                    max_depth= 2,
                    bootstrap= True,
                    max_features= 4,
...
##
def RF(df,Target,estimators= 0,min_samples = 0,max_depth= 0,bootstrap= 0,max_features= 0,criterion= 0,min_samples_split= 0):
    ...
    # Set out model
    if(min_samples == 0):
        clf = RandomForestClassifier(n_estimators=estimators,n_jobs=6,verbose=1)
    elif(min_samples != 0):
    # If Parameters are Selected
        print('Using user paramerts model')
        clf = RandomForestClassifier(n_estimators=estimators,n_jobs=6,verbose=1,min_samples_leaf = min_samples, max_depth=max_depth, bootstrap=bootstrap, max_features=max_features,  criterion =criterion, min_samples_split=min_samples_split)
